...
def make_posterior_fn(loc_init_std=1, scale_init_range=(0.01, 1)):

    def _posterior_fn(dtype, shape, name, trainable, add_variable_fn):
        loc = add_variable_fn(name=name + '_loc',
                            shape=shape,
                            initializer=tf.initializers.random_normal(mean=0.0, stddev=loc_init_std),
                            dtype=dtype,
                            trainable=trainable)
...
class DenseRandomEffects(tfplayers.DenseFlipout):
    def __init__(self, 
        ...
        super(DenseRandomEffects, self).__init__(units=units,
                                                ...
                                                trainable=trainable,
                                                kernel_posterior_fn=make_posterior_fn(loc_init_std, scale_init_range),
