'''
Train adversarial autoencoder on melanoma cell images. 
-----
Kevin Nguyen
1/18/2021
'''
...
OUTPUTDIR = '/archive/bioinformatics/DLLab/KevinNguyen/results/MEDL/melanoma_autoencoder_filtered_lessreg_20210121'
...
autoencoder, adversary, encoder = aae.adversarial_autoencoder(n_latent_dims=LATENT, recon_loss_weight=0.9, adv_loss_weight=0.1)
...
for iEpoch in range(EPOCHS):
    print('----- EPOCH', iEpoch, '-----\n', flush=True)
    # Train on each batch
    for iBatch, (arrBatch, _) in tqdm(enumerate(train_data), total=len(train_data)):
        ...
        # Train autoencoder 
        fAeLossTrain, _, _, fAeMSETrain, fAeAccTrain = autoencoder.train_on_batch(arrBatch, [arrBatch, arrYReal])
    ...
    if iEpoch % VALFREQ == 0:
        ...
        fAeMSEVal = 0
        fAeAccVal = 0
        for iBatchVal, (arrBatchVal, _) in enumerate(val_data):
            ...
            _, _, _, mse, acc = autoencoder.evaluate(arrBatchVal, [arrBatchVal, arrYReal], verbose=0)
            fAeMSEVal += mse
            fAeAccVal += acc
        ...
        arrBatchVal = val_data[0][0]
        arrReconVal, _ = autoencoder.predict(arrBatchVal)
        for iImg in range(8):
