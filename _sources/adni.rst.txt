...
In this application of ARMED, we classify individuals into Alzheimer's Disease
...
1. Image selection: ``select_images_dx.ipynb`` performs some dataset curation and
   ...
   use with the DLLabPipeline.

2. Preprocessing pipeline: Run DLLabPipeline using the config_ADNI23_sMRI.ini
   and the previously generated sMRI_input_files.csv. 

3. Get cross-hippocampal coronal slices: ``preproc_extract_hippo_slices.py``
   extracts a coronal slice through the right hippocampus from each preprocessed
   image. The slices are saved as .png files.

4. Partition data into train/validation/test/heldout sites:
   ``png_to_numpy_data_splits.py`` selects a number of sites to include for
   ...
   performance to sites unseen during training. 
...
.. math:: 
   ...
   h_F(X; \beta)&: \text{latent output from penultimate dense layer}

   \newline
   \text{Adversarial clasifier:}
   
   \hat{z}&: \text{predicted cluster membership}
   ...
   h_R(X; U(z))&: \text{random effects subnetwork output}
...
A classic convolutional neural network with 7 convolution blocks (convolution ->
batch normalization -> PReLU -> max pooling) and two final dense layers. See
blue area in diagram above.
...
A "naive" approach to incorporating cluster membership into the CNN, where $`Z`$
is concatenated into the model before the penultimate dense layer ($`h_F(X;
\beta)`$ in diagram above).
...
Domain adversarial CNN which adds an adversarial classifier (gray area in
...
prediction task using features minimally associated with cluster effects. 
...
Mixed effects CNN which builds upon the domain adversarial CNN. A random effects
...
subnetwork output before the final sigmoid activation. 
